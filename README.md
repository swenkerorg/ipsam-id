...
![CodeQL](https://github.com/swenkerorg/ipsam-id/workflows/CodeQL/badge.svg)
![CI test](https://github.com/swenkerorg/ipsam-id/workflows/ci-test/badge.svg)
[![Coverage Status](https://coveralls.io/repos/github/swenkerorg/ipsam-id/badge.svg?branch=main)](https://coveralls.io/github/swenkerorg/ipsam-id?branch=main)
...
```bash
npm i @swenkerorg/ipsam-id

# pnpm
pnpm i @swenkerorg/ipsam-id

# yarn
yarn add @swenkerorg/ipsam-id
```

```ts
// es6 module
import { extract } from '@swenkerorg/ipsam-id'
```

### Deno

```ts
import { extract } from 'https://esm.sh/@swenkerorg/ipsam-id'

// deno > 1.28
import { extract } from 'npm:@swenkerorg/ipsam-id'
```
...
Passing requests to proxy is useful while running `@swenkerorg/ipsam-id` on browser. View [examples/browser-article-parser](examples/browser-article-parser) as reference example.
...
```js
import { extractFromHtml } from '@swenkerorg/ipsam-id'
...
In `@swenkerorg/ipsam-id`, `transformation` is an object with the following properties:
...
```ts
import { addTransformations } from '@swenkerorg/ipsam-id'
...
```js
import { removeTransformations } from '@swenkerorg/ipsam-id'
...
In this scenario, `@swenkerorg/ipsam-id` will execute both transformations, one by one:
...
`@swenkerorg/ipsam-id` uses [sanitize-html](https://github.com/apostrophecms/sanitize-html) to make a clean sweep of HTML content.
...
There are 2 methods to access and modify these options in `@swenkerorg/ipsam-id`.
...
```bash
git clone https://github.com/swenkerorg/ipsam-id.git
